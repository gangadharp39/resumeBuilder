...
const ExperienceForm = ({ data, onChange }) => {
    ...
    const generateDescription = async (index) => {
        ...
        try {
            const { data } = api.post('api/ai/enhance-job-desc', {userContent :prompt}, { headers: { Authorization: token }})
  ...
  return (
    <div className='space-y-6'>
      ...
      {data.length === 0 ? (
        <div className='text-center py-8 test-gray-500'>
            ...
            {data.map((experience, index)=> (
                <div key={index} className='p-4 border border-gray-200 rounded-lg space-y-3'>
                    ...
                    <label className='flex items-center gap-2'>
                        <input type="checkbox" checked={experience.is_current || false} onChange={(e)=> {updateExperience(index, "is_current", e.target.checked ? true : false)}} className='rounded border-gray-300 text-blue-600 focus:rong-blue-500' />
                    ...
                    <div className='space-y-2'>
                        ...
                        <textarea value={experience.description || ""} onChange={(e) => updateExperience(index, "description", e.target.value)} rows={4} className='w-full text-sm px-3 py-2 rounded-lg resize-none' placeholder='Describe your key responibilities and achivements...' />
                    </div>
                </div>
            ))}
        </div>
